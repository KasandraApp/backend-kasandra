...
describe('api contract alignment', () => {
    ...
    it('normalizes cash and inventory list payloads with snake_case fields', async () => {
        ...
        expect(cashResult.data.transactions[0]).toMatchObject({ transaction_date: '2026-07-18', category: 'Penjualan' });
        expect(cashResult.data.transactions[0]).not.toHaveProperty('transactionDate');
        expect(inventoryResult.success).toBe(true);
        expect(inventoryResult.data.items[0]).toMatchObject({ item_name: 'Ayam', days_remaining: 2, severity: 'critical' });
        expect(inventoryResult.data.items[0]).not.toHaveProperty('itemName');
    });
