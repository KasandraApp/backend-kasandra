...
export function calculateCurrentBalance(transactions: CashTransactionInput[]): number {
...
export function findDeficitDay(projection: CashProjectionPoint[]): number | null {
  const deficit = projection.find((point) => point.day > 0 && point.value < 0);
  return deficit ? deficit.day : null;
}

export function calculateWeeklyChangePercent(
  currentBalance: number,
  balanceWeekAgo: number,
): number {
  if (balanceWeekAgo === 0) {
...
export function projectInventoryItems(
  ...
  return items.map((item) => {
    const daysRemaining =
      item.averageSalesPerDay > 0 ? item.currentStock / item.averageSalesPerDay : Infinity;

    let severity: InventoryProjectionItem['severity'] = 'info';
...
export function applyWhatIfParameters(
  baseIncome: number,
  baseExpense: number,
  ...
  const avgDailyIncome =
    parameters.avgDailyIncome ?? baseIncome * (1 - incomeDecrease / 100);
  const avgDailyExpense =
    parameters.avgDailyExpense ?? baseExpense * (1 + expenseIncrease / 100);

  return { avgDailyIncome, avgDailyExpense };
}

export function classifySimulationScenario(projectedBalance: number, baselineBalance: number): {
  label: string;
  delta: number;
} {
...
export function generateAlerts(params: {
  ...
  inventoryProjection: InventoryProjectionItem[];
}): AlertCandidate[] {
